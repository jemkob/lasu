  ...
  <div class="box-header with-border noprint">
    ...
    <div class="box-body">
            <div class="row noprint">
                    <div class="col-sm-4">
                      <form action="{{url('mms2/search')}}" method="post">
                        {{csrf_field()}}
                  ...
                  <table border="1" width="100%">
                    <tr>
                      ...
                      <th align="center" rowspan="2">
                          <?php
                          //display courses
                          $themat = $results[2]->matricno;
                                       
                          $rescollection = collect($resultaddup);
                          $resfiltered = $rescollection->where('matricno', $themat)->sortby('subjectcodeco');

                          ?>
                          <table border="1" width="100%">
                              <tr> 
                                @foreach($resfiltered as $index=>$resfilt)
                                 <td style="width:47px"><strong>{{$resultaddup[$index]->subjectcodeco}} [{{$resultaddup[$index]->subjectunitco }}{{$resultaddup[$index]->subjectvalueco}}]</strong></td>
                                
                                @endforeach
                                </tr>
                                <tr>
                                    @foreach($resfiltered as $index=>$resfilt)
                                    <td style="width:47px"><strong>TO</td>
                    ...
                    <tr>
                      ...
                      <td>Remark</td>
                          ...
                          @foreach($results as $index=>$result)
                              ...
                              {{-- {{$resultaddup[0]->subjectcodeco}}
                              {{$resultaddup[1]->subjectcodeco}} --}}
                              <table border="1" width="100%">
                              
                                <tr>
                                  @foreach($resfiltered as $index=>$resfilt)
                                  <td style="width:47px">{{$resfilt->ca+$resfilt->exam}}</td>
                                  @endforeach
                                </tr>
                              </table></td>

                              {{-- Begin Current cgpa, tnu, tnup --}}
                              <?php
                              //first department
                              $themat = $result->matricno;
